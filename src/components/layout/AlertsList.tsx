...
type AlertsListProps = {
  ...
  onAlertsChange?: (alerts: AlertResponse[]) => void;
};

export function AlertsList({ anchorEl, open, onClose, alerts, onAlertsChange }: AlertsListProps) {
  const { t } = useI18n();

  const [localAlerts, setLocalAlerts] = useState<AlertResponse[] | null>(alerts);

  useEffect(() => {
    setLocalAlerts(alerts);
  }, [alerts]);

  const loading = localAlerts === null && open;

  const handleAlertClose = async (alert: AlertResponse) => {
    await AlertsService.resolveAlert(alert.id);
    if (localAlerts) {
      const updated = localAlerts.filter(a => a.id !== alert.id);
          ...
          .map((alert) => (
            ...
            >
              ...
              >
                ...
                <IconButton size="small" onClick={() => handleAlertClose(alert)}>
